...
public class BasicStreamFunctions {
    ...
    public static void main(String[] args) {
        ...
        // Answer 2 goes here
        Predicate<Car> isBlueColorCar = (car) -> car.color.equals("blue");

        List<String> blueCars = cars
                .stream()
                .filter(isBlueColorCar)
                .map((Car) -> Car.make)
                .collect(Collectors.toList());
        ...
        // Answer 3 goes here
        Function<Employee, Float> getEmployeeSalary = (employee) -> employee.salary;
        BinaryOperator<Float> sumSalaries = Float::sum;

        Float sumOfEmployeesSalaries = employees
                .stream()
                .map(getEmployeeSalary)
                .reduce(0f, sumSalaries);

        System.out.println(sumOfEmployeesSalaries);
